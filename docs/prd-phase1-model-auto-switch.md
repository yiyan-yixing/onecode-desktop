...
| 6 | 高级字段（自定义供应商） | **上下文窗口已做（2026-09-23）**，其余 backlog | 自定义表单只做四要素：名称 / Base URL / API Key / 型号；代理、并发等高级字段不做。**上下文窗口例外**：触发条件已达成（董事长实测两起 provider 400，见 `docs/incident-20260923-context-overflow.md`），已作为**可留空**字段落地——留空则按型号后缀 / 内置表 / 保守默认自动推导，四要素流程不变 |
...
| O3 | 自定义供应商高级字段（代理/并发/上下文窗口） | **已触发（2026-09-23）**：董事长实测两起 provider 400 → 上下文窗口已落地，代理/并发仍 backlog | @vp-eng（PM） |
